...
class  GCodeGraphics(Lcnc_3dGraphics, _HalWidgetBase):
    def __init__(self, parent=None):
        ...
        self._previewFeedColor = QColor(0, 0, 0, 0)
        self._previewRapidColor = QColor(0, 0, 0, 0)
        self._flipDisplay = False
    ...
    # override user plot - apply Y-axis flip here if Flip_display is enabled
    def user_plot(self):
    ...
    # Preview feed color (static gcode preview lines, feed/arc moves)
    def getPreviewFeedColor(self):
        return self._previewFeedColor
    def setPreviewFeedColor(self, value):
        self._previewFeedColor = value
        if value.alpha() != 0:
            self.colors['straight_feed'] = (value.redF(), value.greenF(), value.blueF())
            self.colors['arc_feed'] = (value.redF(), value.greenF(), value.blueF())
            self.updateGL()
    def resetPreviewFeedColor(self):
        self._previewFeedColor = QColor(0, 0, 0, 0)
    Preview_feed_color = pyqtProperty(QColor, getPreviewFeedColor, setPreviewFeedColor, resetPreviewFeedColor)

    # Preview rapid color (static gcode preview lines, traverse/G0 moves)
    def getPreviewRapidColor(self):
        return self._previewRapidColor
    def setPreviewRapidColor(self, value):
        self._previewRapidColor = value
        if value.alpha() != 0:
            self.colors['straight_traverse'] = (value.redF(), value.greenF(), value.blueF())
            self.updateGL()
    def resetPreviewRapidColor(self):
        self._previewRapidColor = QColor(0, 0, 0, 0)
    Preview_rapid_color = pyqtProperty(QColor, getPreviewRapidColor, setPreviewRapidColor, resetPreviewRapidColor)

    # Flip display upside down (for bottom-up machines e.g. selective solder)
    def getFlipDisplay(self):
        return self._flipDisplay
    def setFlipDisplay(self, state):
        self._flipDisplay = state
        self.updateGL()
    def resetFlipDisplay(self):
        self._flipDisplay = False
        self.updateGL()
    Flip_display = pyqtProperty(bool, getFlipDisplay, setFlipDisplay, resetFlipDisplay)
